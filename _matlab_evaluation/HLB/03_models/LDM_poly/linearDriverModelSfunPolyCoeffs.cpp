...
#define S_FUNCTION_NAME emg_linearDriverModel
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
   ...
   // get output pointers
   int outputIndex = 0;
   real_T *trajEgoFrame = ssGetOutputPortRealSignal(S, outputIndex++);
   ...
   Dc::Emg::TrajectoryPolynomial y = m_obj->runPoly(
           corrX_in,
           corrY_in,
           corrC1_in,
           corrC2_in,
   ...
   for (int i{0}; i < 300; i++){
       trajEgoFrame[i] = y.trajectory.trajectoryPoints[i].PointsX.value();
       trajEgoFrame[i+300] = y.trajectory.trajectoryPoints[i].PointsY.value();
   };
   
   trajEgoFrame[600] = static_cast<vfc::uint16_t>(y.trajectory.trajectoryLength);
   
   trajCoefficients[0] = y.polynomialCoeffs.c0;
   trajCoefficients[1] = y.polynomialCoeffs.c1;
   trajCoefficients[2] = y.polynomialCoeffs.c2;
   trajCoefficients[3] = y.polynomialCoeffs.c3;

   if (DATALOGGING)
   {
      m_generateMeasurementFile->writeDataD97(vfc::CSI::si_second_f32_t {0.01f});
   }
}
