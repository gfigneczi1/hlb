...
#define S_FUNCTION_NAME linearDriverModel

// user parameters
#define NINP 3          // number of Simulink-level inputs (without parameter port)
#define NOUP 1          // number of Simulink-level outputs
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
   ...
   int inputIndex = 0;
   InputRealPtrsType corridorInfo = ssGetInputPortRealSignalPtrs(S, inputIndex++);
   InputRealPtrsType egoPoseGlobal = ssGetInputPortRealSignalPtrs(S, inputIndex++);
   InputRealPtrsType LDMParamIn = ssGetInputPortRealSignalPtrs(S, inputIndex++);

   // get output pointers
   int outputIndex = 0;
   real_T *trajectoryEgoFrame = ssGetOutputPortRealSignal(S, outputIndex++);
