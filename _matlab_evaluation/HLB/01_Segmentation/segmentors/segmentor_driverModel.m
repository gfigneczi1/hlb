...
% measurements
    ...
    testSignals = ["c01_left", "c01_right","c1","c2", "Left_Index", "Right_Index"]; % signals only available in a test vehicle
    ...
    for fileID = 1:length(matFiles)
        ...
        for i=1:lapnumber
            ...
            if (mandatoryStatus && testStatus)
                ...
                rawData = extend(rawData,"Left_Index",1,6);
                rawData = extend(rawData,"Left_Index",1,-3);
                rawData.invalidC0 = ((abs(0.5*(rawData.c01_left+rawData.c01_right)) > 0.8)+(isnan(rawData.c01_left))+(rawData.c01_left==0)+(rawData.c01_left>3.5)+(rawData.c01_right<-3.5)) > 0;
                rawData.invalidC1 = abs(rawData.c1) > 0.05; 
                rawData.invalidSpeed = rawData.VelocityX_ESP < 5;
                rawData = extend(rawData,"invalidC0",1,-3);
                rawData = extend(rawData,"invalidC0",1,3);
                rawData = extend(rawData,"invalidC1",1,-3);
                rawData = extend(rawData,"invalidC1",1,3);
                
                rawData.isStraight = abs(movmean(rawData.c2,100)) <= 2.5e-4;
