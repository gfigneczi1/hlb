...
for fileID = 1:length(segments.segments)
    ...
    [N, edges] = histcounts(offsetError,'BinWidth',0.05);

    % Calculate the theoretical CDF values for each distribution
    pdf_normal = cdf(params_normal, offsetError);
...
global segment_m indexes
 ...
 y_mu = 1/(params_normal(2)*sqrt(2*pi()))*exp(-0.5*(params_normal(1)/params_normal(2))^2);
