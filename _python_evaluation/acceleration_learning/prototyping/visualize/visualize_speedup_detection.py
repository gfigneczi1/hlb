...
# Import the data
expected_timing_file_json: str = f"{pathlib.Path(__file__).resolve().parent.parent.parent}\\tests\speed_up_moments.json"
...
for trip_key, trip_properties in expected_timings.items():
    ...
    speed_ups, stat_velocities, removed_speed_ups_dict = extract_speed_ups(
        data=data_frame,
        trip_name=filename,
        signal_profile=cadillac_signal_profile
    )
    if len(speed_ups):
        ratio = (len(removed_speed_ups_dict["filter_acc"]) + len(removed_speed_ups_dict["filter_non_free_driving"]) + len(removed_speed_ups_dict["anomaly"])) / len(speed_ups)
        print(f"F:{filename}:ratio filter/raws:{ratio}")
    else:
        print(f"F:{filename}: no speed ups")
    ...
    # Speed up
    plot_velocity(speed_ups, "green", "Speed ups", ax)
    ...
    if "filter_acc" in removed_speedups_dict:
        plot_velocity(removed_speedups_dict["filter_acc"], "red", "ACC filtered out speed ups", ax)
    ...
    if "filter_non_free_driving" in removed_speedups_dict:
        plot_velocity(removed_speedups_dict["filter_non_free_driving"], "grey", "Vehicle ahead filtered out speed ups", ax)

    # Anomalies
    if "anomaly" in removed_speedups_dict:
        plot_velocity(removed_speedups_dict["anomaly"], "pink", "Anomalies filtered out speed ups", ax)
