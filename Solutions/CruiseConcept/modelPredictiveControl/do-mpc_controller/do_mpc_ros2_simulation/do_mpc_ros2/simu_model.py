...
class MPCVehicleSimRosNode(Node):
    def __init__(self) -> None:
        super().__init__('mpc_vehicle_simu_node')
        self.publisher_ = self.create_publisher(Float32MultiArray, '/mpc/vehicle_state', 10)
        self.state_subscriber_ = self.create_subscription(Float32, '/mpc/steering_angle', self.callback, 10)
    ...
    def callback(self, msg):
        ...
        # current_error = np.array([[(float(self.mpc.data['_x', 'x3'][-1] - self.x_ref_vec_global[0])**2 + float(self.mpc.data['_x', 'x4'][-1] - self.y_ref_vec_global[0])**2)**0.5, self.count]])
        # self.error_vec = np.concatenate((self.error_vec, current_error), axis=0)

        u0 = np.array([[msg.data]], dtype=np.float32)
        ...
        vehicle_state = Float32MultiArray()
        vehicle_state.data = self.x0.flatten().tolist()
        self.publisher_.publish(vehicle_state)
...
def main(args=None):
    ...
    mpc_vehicle_sim_ros_node = MPCVehicleSimRosNode()
    rclpy.spin(mpc_vehicle_sim_ros_node)
    mpc_vehicle_sim_ros_node.destroy_node()
    mpc_vehicle_sim_ros_node.visualize_results()
    rclpy.shutdown()
