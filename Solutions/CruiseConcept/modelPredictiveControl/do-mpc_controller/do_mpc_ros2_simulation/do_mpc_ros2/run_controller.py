...
class MPCControllerRosNode(Node):
    def __init__(self) -> None:
        super().__init__('mpc_node')
        self.first_save = True
        self.publisher_ = self.create_publisher(Float32, '/mpc/steering_angle', 10)
        self.state_subscriber_ = self.create_subscription(Float32MultiArray, '/mpc/vehicle_state', self.callback, 10)
        timer_period = 0.01  # seconds
        self.timer = self.create_timer(timer_period, self.timer_callback)
        self.count = 0
        df = pd.read_excel('/home/dobayt/ros2_ws/src/do_mpc_ros2/do_mpc_ros2/31_south.xlsx')
        self.x_coordinates = df.iloc[:, 0].values
        self.y_coordinates = df.iloc[:, 1].values
        self.simulation_steps = 12000 # simulation time: simulation_steps * t_step_simulator
        self.ref_road_points_global = np.column_stack((self.x_coordinates, self.y_coordinates))
        model_type = 'continuous'
        self.model = do_mpc.model.Model(model_type)
        # Vehicle Model Parameters 
        self.vx = 17.9 # [m/s]
        self.Calfa_f = 95000 # [N/rad]
        self.Calfa_r = 95000 # [N/rad]
        self.Lr = 1.83 # [m]
        self.Lf = 1.18 # [m]
        self.m = 1400 # [kg]
        self.Iz = 1900 # [kg*m^2]
        ...
        self.kappa = 0
        # control parameters
        self.pred_horizon_mpc = 75 #50
        self.t_step_mpc = 0.011 #0.03
        # for dummy testing
    ...
    def init_controller(self):
        ...
        self.mpc.set_rterm(steering_angle=30)
    ...
    def callback(self, msg):
        ...
        self.x0 = np.array(msg.data, dtype=np.float32).reshape((5, 1))
        #print(self.x0, self.x0.shape)
        self.u0 = self.mpc.make_step(self.x0)
        # Calculate the error distance for debug purposes
        current_error = np.array([[(float(self.mpc.data['_x', 'x3'][self.k] - self.x_ref_vec_global[0])**2 + float(self.mpc.data['_x', 'x4'][self.k] - self.y_ref_vec_global[0])**2)**0.5, self.k]])
        self.error_vec = np.concatenate((self.error_vec, current_error), axis=0)
        #print(self.error_vec)
        # print(float(self.mpc.data['_x', 'x4'][-1]))
        # print((-85.0 > float(self.mpc.data['_x', 'x4'][-1])))
        self.k+=1
        if -85.0 > float(self.mpc.data['_x', 'x4'][-1]) and self.first_save:
            plt.figure()
            plt.plot(self.error_vec[:,1], self.error_vec[:,0])
            plt.savefig('/home/dobayt/ros2_ws/src/do_mpc_ros2/simu_results/error_debug_disturbed3.png')
            plt.figure(2)
            plt.plot(self.x_coordinates[:len(self.mpc.data['_x', 'x4'])], self.y_coordinates[:len(self.mpc.data['_x', 'x4'])])
            plt.plot(self.mpc.data['_x', 'x3'], self.mpc.data['_x', 'x4'], color='r')
            plt.savefig('/home/dobayt/ros2_ws/src/do_mpc_ros2/simu_results/road_pos_debug_disturbed3.png')
            from matplotlib import rcParams
            rcParams['axes.grid'] = True
            rcParams['font.size'] = 18
            fig, ax, graphics = do_mpc.graphics.default_plot(self.mpc.data, figsize=(16,9))
            graphics.plot_results()
            graphics.reset_axes()
            plt.savefig('/home/dobayt/ros2_ws/src/do_mpc_ros2/simu_results/simu_results_disturbed3.png')
            plt.show()
            self.first_save = False


        # steering_angle = Float32()
        # steering_angle.data = self.u0[0][0]
        # self.publisher_.publish(steering_angle)
        # self.get_logger().info('Publishing float: %f' % steering_angle.data)

        # self.count += 1
        # y_next = self.simulator.make_step(self.u0)
...
def main(args=None):
    ...
    mpc_controller_ros_node = MPCControllerRosNode()
    rclpy.spin(mpc_controller_ros_node)
    mpc_controller_ros_node.destroy_node()
    mpc_controller_ros_node.visualize_results()
    rclpy.shutdown()
