function [mu,s2,alpha,yb] = spgp_pred(y,x,xb,xt,hyp,del)
...
% precomputations
tic;
K = kern(xb,xb,hyp) + del*eye(n); % KM
L = chol(K)';
K = kern(xb,x,hyp); % Kmn
V = L\K;
ep = 1 + (kdiag(x,hyp)-sum(V.^2,1)')/sig;   % only use the diagnal part
