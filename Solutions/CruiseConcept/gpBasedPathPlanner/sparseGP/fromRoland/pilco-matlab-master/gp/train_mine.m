...
%
% Train Sparse GP directly, both the inducing point and the nlml
% Last modified: 2020.8
%% High-Level Steps
% # Initialization
% # Train SPGP directly
% # Remember: there should always exist the structure gpmodel.induce  

function [gpmodel nlml2] = train(gpmodel, dump, iter)
%% Code
...
% 2b) If necessary: sparse training using FITC/SPGP (Snelson & Ghahramani, 2006) %
         % are we using a sparse approximation?
...
%      gpmodel.induce(:,:,i) = ctrs;
  ...
  % train sparse model
  [gpmodel.w nlml2] = minimize(gpmodel.w, 'fitc_mine', iter(end), gpmodel);  % fitc
  
  % retrieve 
  for i  = 1:E
      gpmodel.induce(:,:,i) = reshape(gpmodel.w(1:M*D,i),M,D);
      gpmodel.hyp(:,i) = gpmodel.w(M*D+1:end,i);
  end
