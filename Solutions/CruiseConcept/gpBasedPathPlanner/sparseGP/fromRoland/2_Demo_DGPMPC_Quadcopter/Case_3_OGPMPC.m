...
% %
 if flag_update == 1
     model_x.postS = 1e-1*eye(model_x.m);
     model_y.postS = 1e-1*eye(model_x.m);
     model_z.postS = 1e-1*eye(model_x.m);
 end
...
%          target_sp_hist(i,:) =  target_sp;
       ...
       lambda = 0.99;
         ...
         [model_x,model_y,model_z,err] = GP_update_SVGP(mu_xu,target_p,mu_lp,model_x,model_y,model_z,lambda); % err 1x3 vector
