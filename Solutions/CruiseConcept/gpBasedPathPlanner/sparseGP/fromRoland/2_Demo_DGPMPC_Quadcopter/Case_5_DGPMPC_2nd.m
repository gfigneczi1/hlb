%% GPMPC- with online GP compensation in prediction model, without consideration of prediction variance 
...
tt = 0:para.dT:para.T-para.dT;
figure;
plot(tt,winds(1:400,1),'r:','linewidth',2); grid on; hold on 
plot(tt,winds(1:400,2),'b-','linewidth',2);
plot(tt,winds(1:400,3),'k--','linewidth',2);
legend('x','y','z');
xlabel('Times/s');
...
tic;
     for i = 1: para.N-para.trans.H
        ...
        %-----------------------------------------------------------%
          ...
          [u_opt_trans,cost_trans(i)]  = mpcCtrl_trans_DGP_2(xtrans(:,i),xrot(:,i),time(i),para,i); % considering the variance in prediction model
...
%         
%         % long term 
%         [mu_lp1, var_ld1] = gpsvi_predict(cf1.covfunc, par_1.loghyp, par_1.m, par_1.S, par_1.z,  mu_xu);
%         [mu_lp2, var_ld2] = gpsvi_predict(cf2.covfunc, par_2.loghyp, par_2.m, par_2.S, par_2.z,  mu_xu);
%         [mu_lp3, var_ld3] = gpsvi_predict(cf3.covfunc, par_3.loghyp, par_3.m, par_3.S, par_3.z,  mu_xu);
...
%        
       % short term
%         [mu_sp1, var_sd1] = gpsvi_predict(cf1.covfunc, spar_1.loghyp, spar_1.m, spar_1.S, spar_1.z,  mu_xu);
%         [mu_sp2, var_sd2] = gpsvi_predict(cf2.covfunc, spar_2.loghyp, spar_2.m, spar_2.S, spar_2.z,  mu_xu);
%         [mu_sp3, var_sd3] = gpsvi_predict(cf3.covfunc, spar_3.loghyp, spar_3.m, spar_3.S, spar_3.z,  mu_xu);
...
%          var_sd = [var_sd1,var_sd2,var_sd3];

          mu_a = zeros(3,1);  %
          mu_lp = [mu_lp1 mu_lp2 mu_lp3]; %1x3
          mu_sp = [mu_sp1 mu_sp2 mu_sp3];
...
%         target_sp  = getDiffOnline_p_s(x(:,i+1),u(:,i),mu_lp,state_dot_hist(:,i),para,LTI);
       ...
       lambda = 0.995;
         ...
         [Smodel_x,Smodel_y,Smodel_z,err] = GP_update_SVGP(mu_xu,target_sp,mu_sp,Smodel_x,Smodel_y,Smodel_z,lambda); % err 1x3 vector
...
%      norm_S_hist(:,i) = norm(Smodel_x.postS);
%      norm_m_hist(:,i) = norm(Smodel_x.postm);
     end
      toc  
%%
H = para.trans.H;
...
% figure;
% plot(norm_S_hist(1:end-5),'linewidth',2);
% figure;
% plot(norm_m_hist(1:end-5),'linewidth',2);

figure;
subplot(3,1,1);hold on 
plot(target_sp_hist(:,1)); 
plot(mu_sp_hist(:,1));

subplot(3,1,2);hold on 
plot(target_sp_hist(:,2)); 
plot(mu_sp_hist(:,2));

subplot(3,1,3);hold on 
plot(target_sp_hist(:,3)); 
plot(mu_sp_hist(:,3));


save('DGP_2nd.mat','x','u','cost_trans','mu_p_hist','MSE','target_sp_hist','mu_sp_hist','target_p_hist','mu_p_hist','state_dot_hist','LTI');
save('para_2nd.mat','para');