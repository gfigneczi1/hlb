%% lGPMPC- with online GP compensation in prediction model, without consideration of prediction variance 
...
%%
function res = oGPMPC_2nd_for_MC()
...
model_x = para.model_x;
model_y = para.model_y ;
model_z = para.model_z ;
...
%
%  if flag_update == 1
%      model_x.postS = 1e2*eye(model_x.m);
%      model_y.postS = 1e2*eye(model_x.m);
%      model_z.postS = 1e2*eye(model_x.m);
%  end
...
%          
         ...
         [model_x,model_y,model_z,err] = GP_update_SVGP(mu_xu,target_p,mu_lp,model_x,model_y,model_z); % err 1x3 vector
         ...
         norm_S_hist(:,i) = norm(model_x.postS);
...
% 
% figure;
% plot(norm_S_hist(1:end-H),'linewidth',2);
...
% save('lGPMPC_with_update.mat','x','u','cost_trans','mu_p_hist','MSE','target_p_hist','mu_p_hist');
