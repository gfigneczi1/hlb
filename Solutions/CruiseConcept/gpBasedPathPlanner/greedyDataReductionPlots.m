...
matFiles = dir(fullfile("C:\git\KDP\publications\GP\results\greed_check_lower_iterations\KPI_input*.mat"));
...
for i=1:length(matFiles)
    ...
    for shiftID = 1:numel(data.KPI)
        if(USE_NRMS)
            ...
            etaFiles = dir(fullfile("C:\git\KDP\publications\GP\results\greed_check_lower_iterations\ETA_input*.mat"));
...
variables = ["$0.9$", "$0.5$", "$0.375$", "$0.325$", "$0.275$", "$0.25$"];
