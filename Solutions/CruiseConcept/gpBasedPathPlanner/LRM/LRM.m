...
if (~isempty(i0) && ~isempty(i1))
    ...
    for driverID = i0:i1
        ...
        for shiftID=1:size(output,2)
            [RMS, NRMS_W, NRMS_M] = KPIcalculation(y_train(:,shiftID), output_train(:,shiftID));
            [RMS_val, NRMS_W_val, NRMS_M_val] = KPIcalculation(y_val(:,shiftID), output_validation(:,shiftID));
    
            KPI{shiftID} = [RMS, NRMS_W, NRMS_M, RMS_val, NRMS_W_val, NRMS_M_val, trun];
        end
        ETA.P = P;
